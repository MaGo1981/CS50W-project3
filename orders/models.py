...
class Topping(Food):
    ...
    def __str__(self):
        return f"{self.name} - {self.side}, {self.price} eur "
...
class Pizza(Food):
    ...
    def __str__(self):
        return f"{self.name} - {self.size}, with toppings:{self.topping1.name}, {self.topping2.name}, {self.topping3.name}, {self.price} eur"
